...
./plink2_ukbb_vcftopgen --vcf ${1} --memory ${mem} --geno ${maxmiss} --vcf-min-gq ${mingq} --vcf-min-dp ${mindp} --var-filter --set-all-var-ids "@:#:\$r:\$a" --new-id-max-allele-len 10000 m --import-max-alleles 255 --update-sex sex_file.txt --split-par hg38 --make-pgen multiallelics=- --out ${1::-7}
